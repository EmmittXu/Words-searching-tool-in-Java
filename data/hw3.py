...
# Your model is going to consist of two convolutional layers followed by the flattening of the tensor, after
# which you will have two fully-connected layers before a final softmax layer
def test_lenet(learning_rate=0.1, n_epochs=200, dataset='', nkerns=[20, 50], batch_size=500):
    ...
    shared_train_set_x,shared_train_set_y= shared_dataset(datasets[0], borrow=True)
    shared_valid_set_x, shared_valid_set_y = shared_dataset(datasets[1], borrow=True)
    shared_test_set_x, shared_test_set_y = shared_dataset(datasets[2], borrow=True)



    # compute number of minibatches for training, validation and testing
    n_train_batches = shared_train_set_x.get_value(borrow=True).shape[0]
    n_valid_batches = shared_valid_set_x.get_value(borrow=True).shape[0]
    n_test_batches = shared_test_set_x.get_value(borrow=True).shape[0]
